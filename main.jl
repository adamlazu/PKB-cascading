...
for i in 1:nrow(data_fix)
    push!(labels, label(sepal, petal, point1[i], point[i]))
end

display(labels)

function accuracy(predicted_labels, actual_labels)
    if length(predicted_labels) != length(actual_labels)
        throw(ArgumentError("Input label vectors must have the same length"))
    end
...
actual_labels = Tables.matrix(select(data_fix, [:x3]))
acc = accuracy(labels, actual_labels)
